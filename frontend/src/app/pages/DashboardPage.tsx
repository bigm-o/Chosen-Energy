...
export function DashboardPage() {
    ...
    return (
        <div className="space-y-6">
            {/* Header */}
            <div>
                <h1 className="text-3xl font-bold text-gray-900">{getGreeting()}, {user?.fullName?.split(' ')[0] || 'Executive'}! 👋</h1>
                <p className="text-gray-600 mt-1">Here is the comprehensive overview of your operations today.</p>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-5">
                    <div className="flex items-start justify-between">
                        ...
                        <div className="bg-green-50 p-2 rounded-lg"><DollarSign className="w-5 h-5 text-green-600" /></div>
                    </div>
                </div>
                ...
                <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-5">
                    <div className="flex items-start justify-between">
                        <div>
                            ...
                            <p className="text-2xl font-bold text-gray-900">{formatCurrency(stats.grossProfit)}</p>
                            <p className="text-xs text-gray-500 mt-1">Revenue minus Purchases</p>
                        </div>
                ...
                <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-5">
                    <div className="flex items-start justify-between">
                        <div>
                            <p className="text-sm text-gray-600 mb-1">Total Volume Sold</p>
                            <p className="text-2xl font-bold text-gray-900">{formatVol(stats.totalVolume)}</p>
                            <p className="text-xs text-blue-600 mt-1">Total Approved Supplies</p>
                        </div>
                        <div className="bg-blue-50 p-2 rounded-lg"><Fuel className="w-5 h-5 text-blue-600" /></div>
                    </div>
                </div>
                ...
                <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-5">
                    <div className="flex items-start justify-between">
                        <div>
                            <p className="text-sm text-gray-600 mb-1">Total Inward Load</p>
                            <p className="text-2xl font-bold text-gray-900">{formatVol(stats.inwardLoadVolume)}</p>
                            <p className="text-xs text-indigo-600 mt-1">Total Approved Receipts</p>
                        </div>
                        <div className="bg-indigo-50 p-2 rounded-lg"><Package className="w-5 h-5 text-indigo-600" /></div>
                    </div>
                </div>
            </div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-4">
                ...
                <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-4">
                    ...
                    <p className="text-xl font-bold text-gray-900 flex items-center gap-2">
                        {stats.activeTrucks} <Truck className="w-4 h-4 text-blue-500" />
                    </p>
                </div>
                <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-4">
                ...
                <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-4">
                    ...
                    <p className="text-xl font-bold text-gray-900 flex items-center gap-2">
                        {stats.totalCustomers} <Users className="w-4 h-4 text-purple-500" />
                    </p>
                </div>
                <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-4">
                    <p className="text-sm text-gray-600 mb-1">Transloaded Volume</p>
                    <p className="text-xl font-bold text-gray-900 flex items-center gap-2">
                    ...
                    <div className="h-64">
                        <ResponsiveContainer width="100%" height="100%">
                            <AreaChart data={revenueChart}>
                                ...
                                <YAxis axisLine={false} tickLine={false} tickFormatter={(val) => `₦${val / 1000000}M`} tick={{ fontSize: 12 }} />
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                {/* Top Customers */}
                <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
                    <h3 className="text-lg font-semibold text-gray-900 mb-4">Top Customers (Revenue)</h3>
                    <div className="space-y-4">
                        {topCustomers.map((cust, i) => (
                            <div key={i} className="flex items-center justify-between">
                                <div>
                                    <p className="font-medium text-gray-900">{cust.name}</p>
                                    <p className="text-xs text-gray-500">{cust.volume.toLocaleString()} L</p>
                                </div>
                                <span className="font-semibold text-blue-600 text-sm">₦{(cust.revenue / 1000000).toFixed(1)}M</span>
                            </div>
                        ))}
                        {topCustomers.length === 0 && <p className="text-gray-500 text-sm">No customer data available.</p>}
                    </div>
                </div>

                {/* Depot Performance */}
                <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
                    <h3 className="text-lg font-semibold text-gray-900 mb-4">Depot Intake</h3>
                    <div className="space-y-4">
                        {depotPerformance.map((depot, i) => (
                            <div key={i} className="flex items-center justify-between">
                                <p className="font-medium text-gray-900">{depot.depotName}</p>
                                <span className="font-semibold text-indigo-600 text-sm">{depot.volume.toLocaleString()} L</span>
                            </div>
                        ))}
                        {depotPerformance.length === 0 && <p className="text-gray-500 text-sm">No depot data available.</p>}
                    </div>
                </div>
                            ...
                            <div className="space-y-3">
                                {pendingItems.map((item) => (
                                    <div key={item.id} className="flex flex-col p-3 bg-gray-50 rounded-lg border border-gray-100">
                                        ...
                                        <p className="text-xs text-gray-600 mt-1">{item.reference}</p>
